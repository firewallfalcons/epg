...
def format_source_date(date_str, time_str, next_day=False):
    # Combine date and time
    fmt = '%Y-%m-%d %H:%M'
    dt = datetime.strptime(f"{date_str} {time_str}", fmt)
    
    # Original beIN Sports time (No subtraction)
    if next_day:
        dt += timedelta(days=1)
...
def scrape():
    print("Starting beIN EPG Scraper (Original Source Time)...")
        ...
        for idx in range(0, 4):
            ...
            try:
                ...
                for j in range(count):
                    ...
                    # Check if program ends after midnight
                    is_next_day = int(end_t.replace(':', '')) < int(start_t.replace(':', ''))
                    
                    start_xml = format_source_date(date_str, start_t)
                    end_xml = format_source_date(date_str, end_t, next_day=is_next_day)
                    ...
                    if entry_key not in seen_entries:
                        ...
                        # XML entry using +0300 (Source Time)
    ...
    print(f"Successfully saved {FILENAME} using Source Time (+0300).")
